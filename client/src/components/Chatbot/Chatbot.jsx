...
//     e.preventDefault()
//     const response = await fetch(url + '/otp/verifyOTP', {
//       method: 'POST',
//       headers: {
//         'Content-Type': 'application/json'
//       },
...
const Chatbot = ({ setIsOpen }) => {
  ...
  return (
    <div className="chatbot-container">
      ...
      <div className="chatbot-chatwindow">
        {/* <form onSubmit={handleSubmit} className="chatbot-chatwindow-form">
          <Form type={'Email'} value={email} setter={setEmail} />
          <button type="submit">Submit</button>
        </form> */}
        {chatLog.length === 0 && (
          <span className="chatbot-chatwindow-starter">Ask your queries!</span>
        )}
        {chatLog.map((message, index) => (
          <ChatMessage message={message} key={index} />
        ))}
